...
class VarNetDataTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        """
        FastMRI-compatible transform for AdaptiveVarNet (torch>=2.x)
        """
        import torch
        import numpy as np
        from fastmri.data.transforms import to_tensor, apply_mask

        # --- Конвертация базовых полей ---
        kspace_torch = to_tensor(kspace)
        target_torch = to_tensor(target) if target is not None else None

        acq_start = attrs.get("padding_left", 0)
        acq_end = attrs.get("padding_right", kspace_torch.shape[-2])
        crop_size = attrs.get("recon_size", (320, 320))
        max_value = attrs.get("max", 0.0)

        # --- Применяем маску, если задана ---
        if self.mask_func is not None:
            ...
            # Создаём единичную маску, если её нет
